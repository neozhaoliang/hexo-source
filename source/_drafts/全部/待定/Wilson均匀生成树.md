...
**证明**：由于 $G$ 有限且连通，而有限连通图上的简单随机游动是不可约有限马尔可夫链，因此从任意顶点出发，几乎必然在有限时间内命中任意给定的非空顶点集合。所以在算法第 2 步中，每次从一个新顶点 $v$ 出发的随机游动以概率 1 在有限时间内撞到 $T$。这样的循环只能执行有限多次 ( 次数以 $|V|-1$ 为上界 ) ，所以算法以概率 1 在有限时间内结束。$\blacksquare$
...
3.  概率空间 $\Omega$ 是所有栈 $\{S_v\mid v\ne r\}$ 的所有可能的状态组成的集合。这是一个由所有无限栈状态组成的乘积概率空间，其上的概率测度 $\mathbb{P}$ 是各栈元素分布的乘积测度。
:::
...
**证明**：对玩家 $A$ 的操作次数 $n$ 归纳。$n=0$ 时，初始栈顶图已经没有回路，结论显然成立。设 $n\geq1$，并且结论对所有小于 $n$ 的成功弹出过程成立。

设 $A$ 的弹出序列为 $C_1,\ldots, C_n$，玩家 $B$ 第一次弹出的回路是 $D_1$。

若 $C_1=D_1$，则弹出这个回路后，双方到达同一个栈状态。从这个新状态出发，$A$ 可以经过 $n-1$ 次操作后获胜。根据归纳假设，$B$ 无论以后怎样选择，也恰好经过 $n-1$ 次操作获胜，并且弹出的颜色回路与 $\{C_2,\ldots,C_n\}$ 相同。

下面设 $C_1\ne D_1$。由于 $C_1,D_1$ 同时是初始栈顶图中的有向回路，而每个顶点的出度至多为 1，所以两个不同的有向回路必然没有公共顶点。因此弹出 $C_1$ 不会改变 $D_1$ 的任何栈顶元素；弹出 $D_1$ 也不会改变 $C_1$ 的任何栈顶元素。特别地，先弹 $C_1$ 后弹 $D_1$，与先弹 $D_1$ 后弹 $C_1$，得到的是同一个栈状态。
