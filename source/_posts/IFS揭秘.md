...
本文整理自我 2024/06/14 在上海科技大学数学所的一个小报告，标题是「GPU 涂鸦与数学可视化」。这个报告是我和上科大数学所的陈浩老师、Abdelaziz Nait Merzouk 合作的双曲反射群画展期间的一个助兴小节目。感谢陈浩老师一己之力将画展办起来，并带领我去外滩逛了一圈。我在现场拍摄了一些画展的照片，但是由于光线和拍摄技术的原因，照片质量不是很好，就不放在这里献丑了。你可以在 [项目主页](https://github.com/neozhaoliang/Hyperbolic-Honeycombs) 看到原汁原味的渲染图。

以下是报告的技术内容，我略去了关于 demoscene 和分形文化的部分。
...
`fold` 函数背后的原理是所谓的 IFS（Iterated Function System，迭代函数系统），准确的说，它是 IFS 的逆函数。使用 IFS 的逆函数而不是 IFS 本身的原因是，正向的 IFS 是没有办法在 GPU 上并行实现的。当然，要理解这一切，我们还是免不了先从 IFS 开始说起。
...
也就是说，给你一个图形 $X$，我们将它复制成 $N$ 个版本，每个版本分别应用一个压缩变换、再把所有部分合起来，得到新的集合 $F(X)$。
...
然而，同时选择 $N$ 个不同的变换 $f_1,\ldots,f_N$，还要让它们互相协调合作，共同生成漂亮的图案，还是太难做到了。我们后面会介绍，在着色器中绘制 IFS 是通过 $F$ 的逆映射 $F^{-1}$ 来实现的，$F^{-1}$ 就是本文开头提到的「折纸」操作，它是一种「空间折叠」操作，可以通过复合若干“折叠”函数来实现。就像古代的炼丹师会通过反复调整原料配方来寻找效果最佳的丹药一样，设计分形也可以通过调整 $F^{-1}$ 中的折叠函数，并观察屏幕上显示的效果来实现。所以你根本无需关心 $f_i$ 是什么！
...
上面讲到，在着色器里面画 IFS 就是给每个像素指定一个颜色，这个颜色应该由像素对应的点到分形的距离来决定。假设像素对应空间中的点 $p$，初始紧集是 $K$，我们用迭代 $n$ 次的结果 $F^n(K)$ 作为分形的近似，这里 $n$ 是某个常数，在绝大多数场景下 $n=30$ 就足够了。于是我们需要计算距离 $d(p,F^n(K))$ 并根据这个距离值对像素染色。然而，直接计算 $d(p,F^n(K))$ 是不可行的，因为如果有 $N$ 个不同的压缩映射 $f_1,\ldots,f_N$，那么每次迭代后集合的数目会乘以 $N$，这是指数增长的，10 次迭代后就会产生多达 $N^{10}$ 个不同的集合。维护如此数量的集合会轻易耗尽内存。这该怎么办呢？
