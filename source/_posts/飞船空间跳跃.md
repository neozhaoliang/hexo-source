...
:::{.theorem}
定义如下的跳跃策略：在准备第 $n$ 次跳跃时，如果飞船已经在太阳系内，则令 $l_n=0$，否则令 $l_n=|X_{n-1}|-r+\epsilon$，这里 $0<\epsilon<r$。在这个跳跃策略下，飞船返回太阳系的概率大于 $(r-\epsilon)/R$。
:::

注意在这个策略中总是有 $l_n<|X_{n-1}|$，根据 @Pre:supermartingale，$\{Z_n=1/|X_n|\}$ 实际上是一个鞅。此外由于总是有 $|X_n|\geq r-\epsilon$，所以 $Z_n\leq1/(r-\epsilon)$，即 $\{Z_n\}$ 被常数 $1/(r-\epsilon)$ 所控制。
...
$$Y_k=\cos\theta_k,\quad \theta_k\sim{\rm Uniform}[0,2\pi).\label{eq:jump2d}\tag{$\star$}$$
像上次一样，对两边取对数，我们得到
$$\ln |X_n| = \ln R + \frac{1}{2}\sum_{k=1}^n \ln(2+2\cos\theta_k).$$
其中我们记
