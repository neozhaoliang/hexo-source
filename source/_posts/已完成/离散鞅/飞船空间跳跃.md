...
设 $R_n$ 为第 $n$ 次跳跃以后飞船与太阳系的距离，$R_0=R$，我们来推导 $R_n$ 和 $R_{n+1}$ 之间的关系。
...
**证明**：只要再说明 $1/R_n$ 可积即可。由于 $1/R_n$ 非负，因此条件期望 $\E[1/R_n|\mathcal{F}_{n-1}]$ 有定义，并且上面已经证明了它小于等于 $1/R_{n-1}$，于是
$$\E[1/R_n]=\E[\E[1/R_n|\mathcal{F}_{n-1}]]\leq\E[1/R_{n-1}].$$
对 $n$ 归纳即可。$\blacksquare$

@Pre:supermartingale 是解决整个问题最关键的一步，有了它就海阔天空，没有它就寸步难行。由它我们立刻可以导出一个有趣的观察：由于非负上鞅一定是几乎处处收敛的，因此 $\lim\limits_{n\to\infty}R_n(\omega)$ 几乎处处存在。这有两种可能：$\lim\limits_{n\to\infty}R_n(\omega)=+\infty$ 或者 $\lim\limits_{n\to\infty}R_n(\omega)=a<+\infty$。所以飞船要么飞向无穷远，即迷失在宇宙的深处，要么被吸引到某个有限的位置。
...
设 $Z_n=1/R_n$，则 $\{Z_n\}$ 是非负上鞅，所以 $Z_\infty=\lim\limits_{n\to\infty}Z_n$ 几乎处处存在。考虑由停时 $T$ 截断得到的非负上鞅序列 $\{Z_{T\wedge n}\}$，这个序列也是几乎处处收敛的，其极限为
$$\lim_{n\to\infty}Z_{T\wedge n} = \begin{cases}\lim_{n\to\infty}Z_n& T=\infty,\\ Z_T& T<\infty.\end{cases}$$
一方面，根据非负可积函数列的 Fatou 引理有
...
我们不妨假定 $\mathbb{P}(T<\infty)>0$，否则等于 0 的话当然小于 $r/R$，没什么好证的。为此只要证明在事件 $\{T<\infty\}$ 上几乎处处有 $Z_T>1/r$，即 $R_T<r$ 即可。
...
其中上式最右边的期望是对单位球面上均匀分布的 $U_n$ 进行积分，结果是一个关于 $(X_{n-1},l_n)$ 的函数。显然，上式右边作为一个只取 $\{0,1\}$ 两个值的函数的概率积分，结果必然在区间 $[0,1]$ 中。即
$$\E\left[\ind_{\{0,r\}}\circ |X_{n-1}+l_nU_n|\right]\in[0,1].$$
...
根据归纳假设，$|X_{n-1}|=0 \text{ or } |X_{n-1}|=r$ 的概率是 0，即 $\E\left[\ind_{\{0,r\}}\circ |X_{n-1}+l_nU_n|\right]$ 是一个几乎处处为 0 的函数，从而 $\E[\ind_{A_n}|\mathcal{F}_{n-1}]$ 也几乎处处为 0，即得所证。
