...
我们来计算 $I_1(x)$ 的主项。记住，我们只关心主项，常数和误差项不是我们关心的。

由于 $I_1(x)$ 的值与 $x$ 方向无关，我们可以不妨取 $x$ 为正实轴方向。记 $R=|x|$，用代换 $u=R\theta$ 将 $I_1$ 变为在更大的圆盘 $|u|\le R\pi$ 上的积分：
$$I_1(x) = \int_{|u|\le R\pi}\frac{1-\cos u_1}{|u|^2}\,\mathrm{d}u.$$
其中 $u=(u_1,u_2)$，$u_1$ 是 $u$ 的 $x$ 分量。
现在的目的是分析 $R\to\infty$ 时 $I_1$ 的行为。将积分拆成单位圆内 $|u|\le 1$ 和单位圆外 $1<|u|\le R\pi$ 两部分：

+ 在 $|u|\le 1$ 时，由于
$$\lim_{u\to0}\frac{1-\cos u_1}{|u|^2}\sim \frac{u_1^2/2}{|u|^2}\le\frac{1}{2}.$$
被积函数在 $|u|\le1$ 内连续有界，所以这一部分是一个与 $R$ 无关的有限常数，可以不用管。
+ 在 $1<|u|\le R\pi$ 时，

我们借助 [余弦积分](https://mathworld.wolfram.com/CosineIntegral.html)
...
$$\frac{1}{y\cos\delta}\int_1^{y\sin\delta}|\ci(t)|\,\mathrm{d}t\leq \frac{1}{y\cos\delta}\int_1^{y\sin\delta}\frac{2}{t}\,\mathrm{d}t=\frac{2\ln y\sin\delta}{y\cos\delta}\xrightarrow{y\to\infty}0.$$
