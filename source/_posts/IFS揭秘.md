...
有个巧妙的做法可以绕过这个困难：我们把 $d(p,F^n(K))$ 中的 $F^n$ 挪到另一侧并取逆，转而计算 $d(F^{-n}(p), K)$！实际上，如果 $F$ 只包含旋转、平移、反射、缩放、球反演这些变换的话，$d(p,F^n(K))$ 和 $d(F^{-n}(p), K)$ 之间存在非常简单的关系，我们可以通过计算后者来得到前者！这个关系的推导我放在 [注释](#note-F) 中介绍。
