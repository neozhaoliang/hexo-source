...
**证明**：上式两边关于 $f$ 是线性的，所以只要对 $\ell^2(V)$ 的一组基 $\{\ind{x}\mid x\in V\}$　和某个任意的 $\theta$ 验证即可。这时左边是 $c(x)\cdot(\mathrm{div}\,\theta)(x)$，右边是 $\sum_{y\sim x}\theta(x,y)$，根据 $\Div$ 的定义，二者相等。$\blacksquare$
...
现在我们把讨论扩展到无限图。设 $G=(V,E)$ 是一个有可数多个顶点、连通、局部有限的图，局部有限的含义是每个顶点的度数都是有限的。固定一个顶点 $a$，并取 $G$ 的单调递增的、有限、连通子图序列 $\{G_n\}_{n=0}^\infty\uparrow G$ 使得 $G_0=\{a\}$ 以及 $a\in G_n$ 对任何 $n$ 成立。这样的序列总是存在的，例如可以取 $G_n$ 为以 $a$ 为中心，所有与 $a$ 的距离不超过 $n$ 的顶点构成的子图。令 $Z_n=G\setminus G_n$，并把 $Z_n$ 整体视作一个单独的顶点。这样我们就得到了一个有限图序列 $G_n:=G_n\cup\{Z_n\}$。由于从 $a$ 出发的随机游动逃逸到 $Z_{n+1}$ 自然也就逃逸到了 $Z_n$，所以序列 $\{\mathbb{P}(a\to Z_n)\}$ 单调下降，因此极限 $$\mathbb{P}(a\to \infty)=\lim_{n\to\infty}\mathbb{P}(a\to Z_n).$$
...
我们将证明 $\lim\limits_{n\to\infty}i_n$ 存在，并且这个极限也是从 $a$ 到 $\infty$ 的单位流。
...
根据 @Pre:current-by-green，$\mathbb{E}Y_n=\mathcal{G}_{Z_n}(a,x)=c(x)v_n(x)$。所以 $\lim\limits_{n\to\infty}v_n(x)=v_\infty(x)$ 也是存在的。定义 $i_\infty=\nabla v_\infty$，则
...
由于
$$\frac{1}{2}\sum_{e\in G_n}r(e)i_\infty(e)^2=\lim_{l\ge n}\frac{1}{2}\sum_{e\in G_n}r(e) i_l(e)^2\le \varlimsup_{l\ge n}\,\mathcal{E}(i)\le \mathcal{R}_{\rm eff}(a\to Z_n)\leq\infty.$$
左边对 $n$ 一致有界且收敛到 $\mathcal{E}(i_\infty)$。$\blacksquare$

# 应用

:::{.theorem}
**Nash-Williams 判定** 设 $\mathcal{N}$ 是一个有限或者可数、局部有限、连通网络，$\{\Pi_j\}$ 是一组互不相交的割集，并且每个割集有限，则
$$\mathcal{R}_{\rm eff}(a\to\infty)\ge \sum_j \frac{1}{\sum_{e\in \Pi_j}c(e)}.$$
:::

**证明**：只要证对任何单位流 $\theta$ 有
$$\mathcal{E}(\theta)\ge\sum_j \frac{1}{\sum_{e\in \Pi_j}c(e)}.$$
然后对所有 $\theta$ 取下确界即可。
...
:::{.lemma}
对任何割集 $\Pi$ 有
$$\sum_{e\in \Pi}|\theta(e)|\geq \|\theta\|.$$
:::

假设引理成立，那么利用 Cauchy-Schwartz 不等式，有
$$1\le \left(\sum_{e\in\Pi_j}|\theta(e)|\right)^2\le \left(\sum_{e\in\Pi_j}c(e)\right)\left(\sum_{e\in\Pi_j}r(e)\theta(e)^2\right).$$
于是
$$\mathcal{E}(\theta)\ge\sum_j\sum_{e\in\Pi_j}r(e)\theta(e)^2\ge\sum_j\frac{1}{\sum_{e\in\Pi_j}c(e)}.$$

回到引理证明。记 $K$ 是删掉 $\Pi$ 之后，剩下的含有 $a$ 的连通分量。则
$$\sum_{x\in K}\sum_{y\sim x}\theta(x, y) = \|\theta\|.$$
另一方面，$K$ 内部的边在这个求和中出现两次，方向相反，从而抵消。剩下的只有从 $K$ 出发到 $\Pi$ 的边，所以上式还等于
$$\sum_{x\in K}\sum_{e=(x,y)\in\Pi}\theta(x,y)=\sum_{e\in\Pi}\theta(e)\le \sum_{e\in\Pi}|\theta(e)|.$$