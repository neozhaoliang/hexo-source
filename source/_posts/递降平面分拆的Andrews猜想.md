...
另一方面，@Prev:Andrews-conjecture 给出的递推关系是 [^1]
$$\prod_{1\leq i\leq j\leq n+1}\frac{1-q^{n+i+j}}{1-q^{2i+j-1}}\,\bigg/ \prod_{1\leq i\leq j\leq n}\frac{1-q^{n+i+j-1}}{1-q^{2i+j-1}} = \prod_{i=1}^n\frac{1-q^{2n+i+1}}{1-q^{n+i}}=\frac{\binom{3n+1}{n}_q}{\binom{2n}{n}_q}.$$
...
所以我们只要证明
$$1 + \sum_{k=1}^n\frac{h_{kn}}{h_{0n}}=\frac{\binom{3n+1}{n}_q}{\binom{2n}{n}_q}.$$
即可验证 Andrews 猜想成立。
...
\label{eq:hn}\tag{1}
$$
观察上式，看起来 $h_{0n}$ 孤零零地在右边，这不美观啊？可不可以把它和 $h_{1n},\ldots,h_{nn}$ 放在一起呢？这就是 Mills 等人的精彩操作了：考虑
$$
...
$$
上面右边向量的第一个分量我们不知道，但它**应该**是什么是显而易见的，肯定是 $h_{0n}$ 啊！这不就是一个特征值是 1 的特征向量嘛？所以我们下面的任务肯定是构造合适的 $K_n$，使得
$$K_n\begin{pmatrix}h_{0n}\\h_{1n}\\\vdots\\h_{nn}\end{pmatrix}=
  \begin{pmatrix}h_{0n}\\h_{1n}\\\vdots\\h_{nn}\end{pmatrix}.
$$
成立，然后求解它特征值为 1 的特征向量啊！
...
我们断言向量 $(v_{0n},\ldots,v_{nn})$ 和 $(h_{0n},\ldots,h_{nn})$ 必然是共线的。这是因为，如果适当缩放前者使得 $v_{0n}=h_{0n}$ 的话，那么 $(v_{1n},\ldots,v_{nn})$ 也是 $(\ref{eq:hn})$ 中关于 $\{h_{kn}\}_{k\geq1}$ 的线性方程组的解，而由于 $A_n$ 可逆，该方程组的解唯一，所以对 $k\geq1$ 同样有 $v_{kn}=h_{kn}$。于是
$$1+\sum_{k=1}^n\frac{h_{kn}}{h_{0n}}=\frac{\sum_{k=0}^nv_{kn}}{v_{0n}}=\frac{\binom{3n+1}{n}_q}{\binom{2n}{n}_q}.$$
...
$$
两边 $x^n$ 的系数，取第一个因子的 $x^k$ 项系数，第二个因子的 $x^{n-k}$ 系数并对 $k$ 求和即得。
...
$$
对分母，分离 $j=n+1$ 的部分有
$$y_{n+1}=y_n\cdot \prod\limits_{i=1}^{n+1}(1-q^{2i+n}).$$
对分子，分离 $j=n+1$ 的部分有
$$
\begin{aligned}
x_{n+1}&=\prod_{i=1}^n\prod_{j=i}^n(1-q^{n+i+j})\cdot \prod_{i=1}^{n+1}(1-q^{2n+i+1})\\
&=\prod_{i=1}^{n-1}\prod_{j=i}^n(1-q^{n+i+j})\cdot (1-q^{3n}) \cdot \prod_{i=1}^{n+1}(1-q^{2n+i+1})\\
&=\prod_{i=2}^n\prod_{j=i-1}^n(1-q^{n+i+j-1})\cdot (1-q^{3n}) \cdot \prod_{i=1}^{n+1}(1-q^{2n+i+1})\\
&=\prod_{i=2}^n\prod_{j=i}^n(1-q^{n+i+j-1})\cdot \prod_{i=2}^n(1-q^{n+2i-2})\cdot (1-q^{3n})\cdot \prod_{i=1}^{n+1}(1-q^{2n+i+1})\\
&=\prod_{i=2}^n\prod_{j=i}^n(1-q^{n+i+j-1})\cdot \prod_{i=1}^{n}(1-q^{n+2i})\cdot \prod_{i=1}^{n+1}(1-q^{2n+i+1})\\
&=x_n\cdot\prod_{i=1}^{n}(1-q^{n+2i})\cdot\prod_{i=1}^{n+1}(1-q^{2n+i+1})\,\bigg/\prod_{j=1}^n(1-q^{n+j}).
\end{aligned}
$$