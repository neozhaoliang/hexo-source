...
现在我们把讨论扩展到无限图。设 $G=(V,E)$ 是一个连通的、局部有限的图，即每个顶点 $x$ 的度数都是有限的。我们考虑 $G$ 的单调递增的子图序列 $G_n\uparrow G$ 使得 $x\in G_n$ 对任何 $n$ 成立。这样的序列总是存在的，例如我们可以取 $G_n$ 为以 $x$ 为中心，所有与 $x$ 的距离不超过 $n$ 的顶点构成的子图。令 $Z_n=G\setminus G_n$