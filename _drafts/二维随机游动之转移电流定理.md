...
**证明**：记 $P_\bigstar$ 为从 $\ell^2_{-}(E)$ 沿着 $\lozenge$ 到 $\bigstar$ 的投影算子，则 $P_\bigstar$ 是自共轭算子，且有 $P_\bigstar^2=P_\bigstar$，从而
$$(i^e, i^{e'})_E=(P_\bigstar\chi^e,P_\bigstar\chi^{e'})_E=(P_\bigstar\chi^e,\chi^{e'})_E=(i^e,\chi^{e'})_E=r(e')i^e(e').$$
$\blacksquare$

:::{.definition #def-transfer-current}
记 $Y(e,e')=i^e(e')$，矩阵 $\big(Y(e,e')\big)_{e,e'\in E}$ 叫做转移电流矩阵。$Y(e,e')$ 满足互反律
$$Y(e,e')r(e')=Y(e',e)r(e)=(i^e,i^{e'})_E.$$
:::

:::{.corollary #unit-flow}
设 $x,y$ 是两个相邻的顶点，在所有从 $x$ 到 $y$ 的单位流中，电流的能量最小。并且电流的能量等于 $x,y$ 之间的等效电阻。
:::

**证明**：设 $f,g$ 是任何这样的两个单位流，则 $f-g$ 处处净流量为 0，即 $f-g\in\lozenge$，从而 $P_\bigstar f = P_\bigstar g$。记这个投影为 $i$，则 $i$ 是从 $x,y$ 到 $y$ 的单位流且 $i\in\bigstar$，所以 $i$ 是电流。由于 $i\perp f-i$，所以
$$\mathcal{E}(f)=(f,\,f)_{E} = (i,i)_{E} + (f-i,f-i)_{E}\geq (i,i)_{E}=\mathcal{E}(i).$$
这就证明了电流能量的最小性。为了证明它等于 $x,y$ 之间的等效电阻，可以利用
$$(i,i)_E = (i, \chi^e)_E  = r(x,y)i(x,y)= U(x)-U(y).$$
其中 $U(x)-U(y)$ 是 $x,y$ 之间的电势差，而流入网络的总电流是 1，所以这个电势差等于网络的等效电阻。$\blacksquare$

# 3 概率解释

在本节中，我们来给出电势、电流对应的概率解释。

设 $a$ 是一个顶点，$Z\subset V$ 且 $a\notin Z$。

- 考虑从顶点 $x$ 出发，转移概率为 $P(x,y)=c(x,y)/c(x)$ 的随机游动 $\{X_t,t\geq0\}$，其中 $X_0=x$。$\{X_t\}$ 是可逆 Markov 链，以 $\{c(x),x\in V\}$ 为平稳测度：$$c(x)P(x,y)=c(y)P(y,x).$$

- 在 $a$ 点接上电源正极，**并将 $Z$ 接地**，则电流从 $a$ 流入并从 $Z$ 流出。我们将建立随机游动和这一电网络之间的联系。

## 电压的概率解释 （第一种）

电压的第一个解释是，它是一维随机游动模型中赌徒获胜/破产概率的类比。

考虑从 $x$ 出发的随机游动，设 $\tau_a$ 是这一随机游动首次到达 $a$ 点的时间 （赌徒获胜），$\tau_Z$ 是这一随机游动首次到达集合 $Z$ 的时间 （赌徒输光），并考虑概率 $$h(x) = \mathbb{P}_x(\tau_a<\tau_Z).$$ 其中我们规定 $h(a)=1, h(Z)=0$，则 $h(x)$ 在任何 $x\notin \{a\}\cup Z$ 上是**调和**的：
$$h(x) = \sum_{y\sim x}P(x, y)h(y).$$

假设 $a$ 点加上的电压大小是 1，则其电势 $v(a)=1$，由于 $Z$ 接地所以 $v(Z)=0$。我们断言电势函数 $v(x)$ 在任何一点 $x\notin\{a\}\cup Z$ 处也是调和的： $$v(x) = \sum_{y\sim x}P(x, y)v(y).$$ 这是因为设 $v$ 对应的电流是 $i$，则
...
如果 $a$ 处的电压单位电压的话，则结论中的 $v(x)$ 应替换为 $v(x)/v(a)$。
