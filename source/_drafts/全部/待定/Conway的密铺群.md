...
Conway 等人的论文里面包含了好几个密铺的问题，上面这个问题只是其中一个。虽然这个问题的表述很简单，但它的解法并不“初等”。这里我称之为“”初等”的方法是染色法。染色法是最常用的论证不可能密铺的手段。它可以理解为一种取值于 Abel 群的加性不变量。例如，可以给每一个正六边形赋一个有理数权值，使得一块骨头无论如何摆放，其覆盖的三个正六边形的权值之和总是整数，而整个区域所的权值之和不是整数。等价地说，我们构造了一个取值于 $\mathbb{Q}/\mathbb Z$ 的群同态，它在所有骨头上取 0，但是在待铺区域上不取 0。这样来得出矛盾。

然而，这种染色不变量不能解决所有的 $T_n$。Conway 和 Lagarias 在论文中借助“密铺的同调群”证明了在 $n\equiv 0,8\pmod 9$ 这两类情形中，任何广义染色不变量都无法给出矛盾。

Conway 和 Lagarias 用“密铺的同伦群”给出了不可能密铺的证明。同伦群方法的基本思想是，我们仍然用一个群（未必是 Abel 群）的元素作标记，但是这次是给区域和瓷砖的**边界**作标记，来获得密铺的某种不变量，并说明 $T_n$ 的**边界**不满足这个不变量，从而导出矛盾。本文就来介绍这一证明。
...
设 $D$ 是 $\mathcal{G}(A)$ 中的单连通有限区域，边界为 $\partial D$，$\partial D$ 是一条简单闭路径。从 $\partial D$ 上任一顶点出发，绕着边界逆时针一周可以得到此路径的对应的一个字 (word) $\pi\in F$。
...
设 $D$ 是 $\mathcal{G}(A)$ 中的单连通有限区域，定义其**组合边界** $[\partial D]$ 为 $F$ 中的一个共轭类：$[\partial D]=\{ w\pi w^{-1} \mid w\in F\}$，其中 $\pi$ 是从 $\partial D$ 上任一顶点逆时针出发绕 $D$ 一圈得到的字。
:::
...
:::{.theorem .unnumbered}
[@Conway1990, thm 2.1] \
设 $D$ 是平面上正六边形网格中的一个有限的、单连通的区域，则 $D$ 可以被若干骨头密铺的必要条件是 $[\partial D]\subseteq T$。
:::
...
现在我们来构造群同态 $\rho:\ J\to\mathbb{Z}^3$。任何 $w\in J$ 都对应 Cayley 图 $\mathcal{G}(T_0)$ 中的一条闭路径 $\gamma$，我们规定 $\rho(w)=(n_1,n_2,n_3)\in\mathbb{Z}^3$，其中 $n_i$ 是 $\gamma$ 关于所有类型为 $C_i$ 的六边形的环绕数之和：
...
由于充分远的六边形都位于 $\mathbb C\setminus \gamma$ 的无界连通分支中，它们的环绕数为 0，所以上面的求和只有有限多项非零。

我们来计算下每种骨头的环绕数。以边界字为 $w_1$ 的骨头为例：

![](/images/tilinggroup/winding.svg){width=300 .fig}

它关于两个 $C_1$ 类型的正六边形分别逆时针和顺时针各转了一圈，合起来绕了 0 圈；它没有环绕过 $C_2$ 和 $C_3$ 类型的正六边形，关于这俩的环绕数都是 0，所以这块骨头对应的三元组是 $(0,0,0)$。对另外两种骨头也是如此。

但是仅仅知道 $w_1,w_2,w_3\in\ker\rho$ 还不够。因为 $T$ 是它们在 $F$ 中生成的正规闭包，我们还需要说明，它们在 $F$ 中的任意共轭仍然属于 $\ker\rho$。

设 $x\in F$，$w\in J$。闭道路 $\gamma_{xwx^{-1}}$ 可以分为三段：沿着 $x$ 从单位元走到顶点 $xJ$，然后在该顶点处走一遍 $w$ 对应的闭道路，最后原路返回单位元。第一段和第三段互为反向路径，因此它们在线积分中的贡献完全抵消。剩下的中间部分正是 $\gamma_w$ 在 $T_0$ 的元素 $xJ$ 所对应的欧氏等距变换下的像。这个等距变换保持六边形的类型，但它可能保持或反转平面的取向。因此 $\rho(xwx^{-1})=\pm \rho(w)$。对 $w_1,w_2,w_3$ 来说它们满足 $\rho(w_i)=0$，所以它们的所有 $F$- 共轭元都属于 $\ker\rho$，因此 $T\subseteq\ker\rho$。 
