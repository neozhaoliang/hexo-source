...
1. $C_1$ 总是可以取为单位球 $B(\mathbf{0},1)$。**并且 $C_1$ 是无界球**。由于实球之间互不相交，所以 $C_2,C_3,C_4$ 都落在 $C_1$ 的有界一侧。
...
4. **$B_1(\a_1,r_1)$ 也是一个无界球**，即 $r_1<0$。我们同样认为 $r_1>0$，并在列方程时将 $B_2$ 与其它球的内积取负：
...
结果如下图所示，$B_i$ 用实线绘制，$C_i$ 用虚线绘制，同一对 $(B_i,C_i)$ 使用同样的颜色。${\rm inv}$ 表示该圆被翻转，以无穷区域为内部。注意到每个 $C_i$ 都位于 $B_i$ 的内部（正半空间），这是因为 $(\alpha_i,\hat{\omega_i})\geq1$，所以它们一定是一个包含另一个。这也解释了为什么求解 $B_1$ 时要用较小的那个半径：它的内部（无界区域）必须包含 $C_1$ 的内部（也是无界区域）。