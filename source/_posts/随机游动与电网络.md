...
存在。这个极限不依赖于序列 $\{G_n\}$ 的选取，这是因为，任何这样的两个不同子图序列都必然交织在一起，因此根据 squeezing 原理，它们的极限是一致的。即 $\mathbb{P}(a\to \infty)$ 是合理定义的。
于是等效电阻的极限
$$\lim_{n\to\infty}\mathrm{Reff}(a\to Z_n)=\lim_{n\to\infty}\frac{1}{c(a)\mathbb{P}(a\to Z_n)}=\Reffinf.$$
同样存在，这正是 $a$ 点到网络无穷远处的有效电阻。

:::{.lemma}
设 $i_n$ 是 $G_n$ 上从 $a$ 到 $Z_n$ 的单位电流，则 $\lim_{n\to\infty}i_n$ 存在，且为 $a$ 点到 $\infty$ 的单位电流。
:::
**证明**：