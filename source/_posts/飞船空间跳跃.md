...
**证明**：记 $X_{n-1}\cdot U_n$ 是三维空间的 Euclidean 内积，我们有
$$|X_n|^2=|X_{n-1}|^2+2l_n|X_{n-1}| (X_{n-1}\cdot U_n)+l_n^2.$$
令 $B_n=\{X_{n-1}\cdot U_n\geq 1/2\}$，则在事件 $B_n$ 上我们有
...
在 @Pre:freeze 中令 $X=X_{n-1},Y=U_n,\mathcal{G}=\mathcal{F}_{n-1},\varphi(X,Y)=\ind_{[1/2,1]}\circ(X\cdot Y)$，有

$$
\begin{aligned}
\mathbb{E}[\ind_{B_n}|\mathcal{F}_{n-1}] &= \mathbb{E}[\ind_{[1/2,1]}\circ(X_{n-1}\cdot U_n)\,\bigg|\,\mathcal{F}_{n-1}]\\
&=\mathbb{E}[\ind_{[1/2,1]}\circ(v\cdot U_n)]\bigg|_{v=X_{n-1}}\\
&=\mathbb{P}[(v\cdot U_n)\geq 1/2] \bigg|_{v=X_{n-1}} \\&= \mathbb{P}[U_n\in \{(x,y,z)\in\mathbb{R}^3:\ z\geq 1/2\}]\\&=\frac{1}{4}.
\end{aligned}
$$
