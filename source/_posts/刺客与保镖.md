...
封锁一个二维格点当然还是需要 4 名保镖。但是，如果你仍然认为需要 4 x 6 = 24 名保镖，那对不起，事情不是这样的。

真正的答案是：总共需要 $4 \times 6 \times 6 = 144$ 名保镖。

多出来的那个因子 $6$，到底从哪儿来的？

核心现象是这样的：

在正六边形房间的情形：沿着不同的路径反射，目标的虚像可以出现在另一个房间中不同的位置：

![](/images/polygon-billiard/chirality-2.svg){width=350 .fig}

由于光路的可逆性，这个话也可以反过来说，即虚像房间中的一点可能来自真实房间中若干个不同的实际位置。我们后面会证明这样的位置最多有 6 种不同的可能。换句话说：
