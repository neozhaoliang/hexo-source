...
$$\mathbb{E}\left[\phi(V_{n-1}\cdot U_n)\mid\mathcal F_{n-1}\right]
= g(V_{n-1}),\quad
g(v):=\mathbb E\big[\phi(v\cdot U_n)\big].$$
...
[^1]: 实际上，对任何 Borel 可测集 $A\subset\mathbb R$ 和 $B\in\mathcal{F}_{n-1}$，取 $\phi=\ind_A$，利用
$$\mathbb{E}[\phi(Y_n)\mid\mathcal{F}_{n-1}]=\mathbb{E}[\phi(Y_n)]=\mathbb{P}(Y_n\in A).$$
以及条件期望的塔式性质，可得
$$\begin{aligned}
\mathbb P(Y_n(\omega)\in A,\,\omega\in B)&=\mathbb E[\ind_B\ind_{\{Y_n\in A\}}]\\
&=\mathbb E\left[\ind_B\mathbb E[\ind_{\{Y_n\in A\}}\mid \mathcal F_{n-1}]\right]\\
&=\mathbb E\left[\ind_B\mathbb P(Y_n\in A)\right]\\
&=\mathbb P(Y_n\in A)\mathbb P(B).
\end{aligned}$$
这正是 $Y_n$ 与 $\mathcal F_{n-1}$ 独立的定义。
