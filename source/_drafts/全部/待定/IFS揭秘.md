...
然而代码短可不代表它们容易看懂。尤其是很多作者还喜欢故弄玄虚，把代码作了混淆处理以增加神秘感。对我来说，这种被人秀了一脸却没搞明白对面是怎么装的逼的感觉让人很不爽。当然我不是抱怨，这种炫技行为本身就是黑客文化的一部分，可以理解。
...
打个比方，把 $X$ 想象成孙悟空，他拔出一把毫毛，变出一些小孙悟空 $\{f_i(X)\}_{i=1}^N$，这些小孙悟空构成的集合就是 $F(X)$。
...
不仅如此，Banach 不动点定理还告诉我们，对 $K(\mathbb{R}^n)$ 中的任何一点 $K_0$（记住 $K_0$ 是紧集），都有
$$\lim_{n\to\infty} F^n(K_0) =A.$$
即初始紧集 $K_0$ 的形状根本不重要——压缩映射的力量会抹去一切“个性”，只留下最终的分形结构，那就是 $A$！
...
你可以很容易看出来为什么 $K_0$ 的选择是不重要的：因为紧集总是有界的，而每次压缩都会导致它变为原来的 1/2，从而逐渐缩小到一个单点，所以 $K_0$ 甚至取成一个点也是可以的。
...
更一般地，我们也可以在一串压缩映射中混入像 `max`, `clamp`，甚至是 `sin`, `cos` 等等非压缩的映射。只要最终的复合函数是压缩的，那就仍然可以得到分形。就像涮火锅的时候你可以放不同的调料一样，适当搭配非压缩映射可以引入别样的风味，让分形看起来更加多彩多样。
:::
...
现实生活中有一个很形象的例子可以帮你理解着色器编程：假设你是一场方阵表演的导演，所有演员排成一个方阵，每个演员可以改变自己衣服的颜色。你的任务是发出合适的指令让每个演员根据自己的位置计算出正确的颜色，使得整个方阵呈现出漂亮的图案。

![](/images/ifs/array.png){.fig width=600}

如果你挨个对每个演员下指令，张三你应该显示红色，李四你应该显示蓝色，等等 ... 对成千上万个演员，还不得把人累死？正确的做法是，你应该同时对所有演员发出相同的指令，比如：“每个人，计算自己和方阵中心的距离，小于 10 米的显示红色，大于等于 10 米的显示黑色”。由于每个演员都是一个单独的 GPU 计算单元，他们可以根据你发出的同一条指令，在极短的时间内（毫秒级）分别计算出各自的颜色。计算完毕后，观众应该会看到一个红色的圆。这种基于相同指令并行计算的工作方式就是 GPU 流水线的机制。
