...
这一项贡献了 $I_1$ 的主项。\
同样使用极坐标代换，第二项等于
$$\frac{1}{\pi^2}\int_{1}^{\pi R} \frac{1}{r} \left( \int_{0}^{2\pi} \cos(r \cos\alpha)\, \mathrm{d}\alpha \right) \,\mathrm{d}r.$$
括号里面等于 $J_0(r)$，是零阶第一类贝塞尔函数。而熟知 $J_0(r)$ 在 $r\to\infty$ 时的渐进阶是 $O(r^{-1/2})$，所以这部分的积分收敛。
