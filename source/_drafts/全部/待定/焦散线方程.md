...
形成焦散的原因是，光线经杯壁反射以后并不是均匀散开的。在某些位置，相邻的反射光线会挤压在一起，使单位长度内通过的光能显著增加，于是形成一条明亮的曲线。
...
$$\begin{align}F(X,Y,t)=0,\\\der{F}{t}(X,Y,t)=0.\end{align}$$
也就是
...
F = (X - x) * denominator - numerator * rx
G = (Y - y) * denominator - numerator * ry
