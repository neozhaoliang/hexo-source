设 $V$ 是域 $F$ 上的有限维向量空间，其对偶空间 $V^\ast$ 定义为从 $V$ 到 $F$ 的全体线性函数组成的向量空间。


作为向量空间，$V^\ast$ 和 $V$ 没有任何区别（至少有限维时如此），$V^\ast$ 并不比 $V$ 多出什么 "好" 的性质来，但是对偶空间确实是非常有用的概念，这里面的道理何在呢？


我认为至少有两个原因：

在表示论中，对偶空间提供了一种从已知的表示出发构造新的表示的重要途径（另一种重要途径是张量积），这是第一个原因。

第二个原因是，对偶空间 $V^\ast$ 作为 $V$ 的 "镜像"，可以实现问题的转化：直观上讲，$V$ 中的高维问题，可以转化为 $V^\ast$ 中的低维问题，或者等价地说，$V$ 中关于商空间的问题（商空间的问题通常比较棘手），可以转化为 $V^\ast$ 中关于子空间的问题（这就比较好办）。
...
\[ V\times V^\ast\rightarrow F:\quad (v,f)\rightarrow f(v)\]

是非退化的。我们先讨论一般的非退化双线性函数的性质。
...
设 $V,W$ 是域 $F$ 上的有限维向量空间，映射 $B(v,w):V\times W\rightarrow F$ 是一个双线性函数，如果 $B$ 如下条件：
...
给定非退化双线性型 $B$，对任一 $v\in V$，可以定义一个 $W$ 上的线性泛函 $\varphi_v$：
\[ \varphi_v(w)=B(v,w).\]
...
总之，如果存在 $V\times W\rightarrow F$ 的非退化双线性型 $B$，则 $\dim V=\dim W$，而且 $V$ 和 $W$ 可以借助于 $B$ 同构地映射为对方的对偶空间，这时我们称 $V$ 和 $W$ 关于 $B$ 互为对偶空间。
...
对任意 $f\in\text{End}(W)$，定义 $V\times W$ 上的双线性函数 $B_f$ 为 $B_f=B(v,fw)$。映射 $f\rightarrow B_f$ 明显是线性的，而且利用 $B$ 的非退化性可见这是一个单射，于是 $f\rightarrow B_f$ 是一个从 $\text{End}(W)$ 到 $\text{Bil}(V,W)$ 的单射。然而 $\text{End}(W)$ 和 $\text{Bil}(V,W)$ 的维数相同，都是 $\dim V\times \dim W$，从而这是同构，因此 $\text{Bil}(V,W)$ 中任何元素都形如 $B(v,fw)$，其中 $f\in\text{End}(W)$ 是唯一确定的。特别地对双线性型 $B(Av,w)$ 应用此结论可知存在唯一的 $A^\ast\in\text{End}(W)$ 使得 $B(Av,w)=B(v,A^\ast w)$ 成立，这就证明了结论。
...
我们前面说过，作为向量空间，$V^\ast$ 与 $V$ 是完全一样的，二者互为 "镜像"。这个说法对伴随变换对 $A$ 和 $A^\ast$ 也成立：作为线性变换，二者也是完全一样的，这是因为 $v\rightarrow\varphi_v$ 是一个 $A-$ 模与 $A^\ast-$ 模的同构。


我们解释一下为什么前面说 $V$ 中的高维问题可以转化为 $V^\ast$ 中的低维问题。通过取商空间，$V$ 中关于高维子空间的问题自然变成了低维商空间的问题，但是带来的问题就是商空间比较难处理，然而借助于非退化双线性型，这个商空间中的问题可以 "跑到" 对偶空间中变成一个（低维）子空间中的问题，这就实现了问题的转化。

我们将举两个例子来说明这一点。


    问题：设 $V$ 是域 $F$ 上的 $n$ 维向量空间，$A$ 是 $V$ 上的线性变换。

    1. 如果 $V$ 中每个非零向量都是 $A$ 的特征向量，求证 $A=c\cdot I$。

    2. 如果对每个超平面 $H\subset V$ 有 $A(H)\subset H$，求证 $A=c\cdot I$。这里超平面是指 $V$ 的 $n-1$ 维子空间。


第一问很简单，我就不再写出证明了。这里的关键在于如何利用第一问快速得出第二问的证明。

如果你已经读懂了前面关于非退化双线性型的知识，这就太简单了：$A$ 以 $V$ 中任何 $n-1$ 维子空间 $H$ 为不变子空间因而诱导出 1 维商空间 $V/H$ 上的数乘变换，这个变换与 $A^\ast$ 在 $H^\bot$ 上的作用是一样的，而 $H^\bot$ 是 $V^\ast$ 中的一维子空间，这就把问题变成对偶空间中的一维子空间了。

细节留给你。


Frobenius 有理标准形定理的证明


设 $A$ 是有限维向量空间 $V$ 上的线性变换，对每个向量 $v\in V$，定义它的阶为 $m_v(x)\in F[x]$，这里 $m_v(x)$ 是使得 $m_v(A)v=0$ 的次数最低的首一多项式。任何使得 $g(A)v=0$ 的多项式 $g(x)$ 都是 $m_v(x)$ 的倍式。
    ...
    是另一种分解为循环子空间的方式，$\{w_1,\ldots,w_s\}$ 的阶也都是素多项式的幂 $\{q_1(x)^{f_1},\ldots,q_s(x)^{f_s}\}$，则 $r=s$ 且两个集合 $\{p_1(x)^{e_1},\ldots,p_r(x)^{e_r}\}$ 与 $\{q_1(x)^{f_1},\ldots,q_s(x)^{f_s}\}$ 只相差一个重排。
...
设 $A^\ast$ 为 $A$ 的伴随变换：$(Av,f)=(v,A^\ast f)$，显然 $A$ 和 $A^\ast$ 有相同的极小多项式。
