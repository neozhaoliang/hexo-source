...
我们观察这张图，想象在空间中鸟瞰它，发现它很像是在墙角“堆箱子”。不仅如此，箱子的堆放方式还满足规律：从墙角开始，沿着两侧墙壁的方向，箱子的高度是弱递减的。

我们可以把这个堆箱子的方式对应到一个 $a$ 行 $b$ 列的矩阵，其中 $a_{ij}$ 表示地面上第 $i$ 行第 $j$ 列位置箱子的高度（习惯上空白位置的 0 不写出来）：
...
> + $A$ 的每一行从左到右，每一列从上到下都是弱递减的。
> + $a_{ij}$ 都是介于 $0$ 和 $c$ 之间的整数（箱子的高度不能超过天花板的高度 $c$）。

我们把矩阵 $A$ 叫做一个参数为 $(a,b,c)$ 的受限制的**平面分拆**。

每个受限制的平面分拆都对应一种堆箱子的方式，从而对应六边形的一个菱形密铺。这个对应是一一的：把三种方向的菱形分别看成正方体的三个可见面，每个密铺便组成一个没有裂缝和悬空部分的阶梯曲面。这个曲面与墙面相接，因此可以看成一堆单位立方体的外表面。

反过来，任意一堆沿两个水平方向高度弱递减、且高度不超过 $c$ 的立方体，其可见表面投影到平面后，都给出六边形的一种菱形密铺。两个过程显然互逆。
...
这里我们统一规定当 $r<0$ 或者 $r>n$ 时，二项式系数 $\binom{n}{r}=0$。
