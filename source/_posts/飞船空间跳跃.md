...
下面这个命题是冻结引理的一个直接应用，会在本文中反复用到，所以值得单独列在这里。

:::{.proposition #random-inn}
设 $U$ 是定义在某概率空间 $(\Omega,\mathcal F,\mathbb P)$ 中，在 $\mathbb R^d$ 的单位球面上均匀分布的随机向量，$\sigma$- 域 $\mathcal G\subset \mathcal F$ 与 $U$ 独立。$V$ 是 $\mathcal G$- 可测的 $\mathbb R^d$ 中的单位向量。记 $Y=U\cdot V$，则对任何有界 Borel 可测函数 $f:\mathbb R\to\mathbb R$ 都有
$$\E[f(Y)\mid\mathcal G]=\E[f(U^{x})] = \E[f(Y)].$$
其中 $U^x$ 表示 $U$ 的 $x$- 分量。由此，我们可以得到以下两个结论：
...
右边是一个常数，它不依赖于 $\omega$，所以它必然还等于 $\E[f(Y)\mid\mathcal G]$ 的期望 $\E[f(Y)]$。这就证明了命题的前半部分。
...
以及条件期望的塔式性质，可得
$$\begin{aligned}
\mathbb P(Y(\omega)\in A,\,\omega\in B)&=\mathbb E[\ind_B\ind_{\{Y\in A\}}]
...
:::{.question .unnumbered}
如果每次总是把飞船的跳跃距离设定为当前飞船到太阳系的距离，即对 $n\geq1$ 有 $l_n=R_{n-1}$，求证
$$\sum_{n=1}^\infty\frac{1}{R_n^2}<\infty,\quad \mathrm{a.e.}$$
:::

**证明**：记 $V_n=X_n/R_n$ 是位置 $X_n$ 的单位方向向量。由 $X_n=X_{n-1}+R_{n-1}U_n$ 可得
$$R_n^2 = |X_{n-1}+R_{n-1}U_n|^2=R_{n-1}^2|V_{n-1}+U_n|^2=R_{n-1}^2(2+2V_{n-1}\cdot U_n).$$
记 $Y_n=V_{n-1}\cdot U_n$。我们来说明 $\{Y_n\}$ 独立、且服从 $[-1,1]$ 上的均匀分布。实际上我们将证明一个更一般的结论：

:::{.lemma #project-u}
设 $V_{n-1}\in\mathbb{R}^3$ 是单位向量，且 $V_{n-1}$ 关于 $\mathcal{F}_{n-1}$ 可测，记 $Y_n=V_{n-1}\cdot U_n$，则 $\{Y_n\}$ 独立、且服从 $[-1,1]$ 上的均匀分布。
:::

**@Pre:project-u 的证明**：设 $\phi:\mathbb{R}\to\mathbb{R}$ 是任一有界可测函数。由于 $V_{n-1}$ 关于 $\mathcal{F_{n-1}}$ 可测，$U_n$ 与 $\mathcal{F}_{n-1}$ 独立，所以我们可以使用 @Pre:freeze，得到


注意在 $\mathbb E\big[\phi(v\cdot U_n)\big]$ 中，$v$ 已经被冻结为单位球面上的固定向量。由于 $U_n$ 均匀分布在单位球面上，而 $v\cdot U_n$ 的分布是旋转不变的。我们可以取 $v=(0,0,1)$，于是 $v\cdot U_n$ 就是 $U_n$ 的 $z$ 分量，当 $U_n$ 是单位球面上均匀分布的向量时，其 $z$ 分量服从 $[-1,1]$ 上的均匀分布。从而
$$g(v)=\frac{1}{2} \int_{-1}^{1} \phi(z)\,\mathrm{d}z.$$
至此我们证明了对任何有界可测函数 $\phi$ 有
$$\mathbb{E}[\phi(Y_n)\mid\mathcal{F}_{n-1}]=\frac{1}{2} \int_{-1}^{1} \phi(z)\,\mathrm{d}z.$$
即 $\mathbb{E}[\phi(Y_n)\mid\mathcal{F}_{n-1}]$ 是与 $\omega$ 无关的常数（这个常数当然就是 $\mathbb{E}[\phi(Y_n)]$）。由 $\phi$ 的任意性，可得 $Y_n$ 与 $\mathcal{F}_{n-1}$ 独立。于是 $\{Y_n\}$ 是 i.i.d 的序列。[^1]

回到原问题的证明。在 $R_n^2 = R_{n-1}^2(2+2Y_n)$ 两边取对数，并迭代可得
$$\log R_n^2 = \log R_0^2 + \sum_{k=1}^n \log(2+2Y_k).$$
根据强大数定律，
$$\frac{1}{n}\sum_{k=1}^n \log(2+2Y_k) \xrightarrow{\rm a.e.}\mu:=\mathbb{E}[\log(2+2Y_1)] = \frac{1}{2} \int_{-1}^{1}\log(2+2z)\,\mathrm{d}z = \log 4-1>0.$$
任取正数 $0<\alpha<\mu$，则对几乎处处的 $\omega$，存在随机变量 $N(\omega)$ 使得当 $n\geq N(\omega)$ 时，
$$\sum_{k=1}^n \log(2+2Y_k)\geq \alpha n.$$
于是对几乎处处的 $\omega$，当 $n\geq N(\omega)$ 时有
$$R_n^2\geq R_0^2e^{\alpha n}\Longrightarrow\frac{1}{R_n^2}\leq \frac{1}{R_0^2}e^{-\alpha n},\quad n\geq N(\omega).$$
因此几乎处处有 $\sum_{n=1}^\infty1/R_n^2<\infty$ 成立。$\blacksquare$

# Star Trek 2

:::{.question .unnumbered}
假设飞船只能在一个固定的二维平面内跳跃，并且和 Star Trek 3 中一样，每次跳跃的距离都等于当前飞船到太阳系中心的距离。飞船还能在有限时间内返回太阳系吗？
:::

和 @Pre:project-u 中一样的道理，我们可以得出
$$R_n^2 =R_{n-1}^2(2+2\cos\theta_n).$$
其中 $\{\theta_n\}$ 是 i.i.d 序列，服从 $[0,2\pi)$ 上的均匀分布。

于是
$$R_n = R_{n-1}\sqrt{2+2\cos\theta_n}=2R_{n-1}\left|\cos\frac{\theta_n}{2}\right|.$$
令 $\phi_n=\theta_n/2$，则 $\{\phi_n\}$ 是 i.i.d 且服从 $[0,\pi)$ 上的均匀分布，并且
$$R_n = 2R_{n-1}|\cos\phi_n|.$$
两边取对数，得到
$$\log R_n = \log R_{n-1} + \log 2|\cos\phi_n|.$$
记 $Y_k =\log 2|\cos\phi_k|$，则有
$$\log R_n = \log R_0 + \sum_{k=1}^n Y_k.$$
利用换元 $t=\frac{\pi}{2}-\phi_k$ 可得
$$\begin{aligned}
&\E Y_k = \ln 2+ \frac{2}{\pi}\int_0^{\pi/2}\ln \sin t\,\mathrm{d}t=0,\\
&\E Y_k^2=\frac{2}{\pi}\int_0^{\pi/2}(\ln 2\sin t)^2\,\mathrm{d}t.
\end{aligned}$$
在 $0$ 附近的小区间 $[0,\delta]$ 上，$\sin t\sim t$，所以
$$\frac{2}{\pi}\int_0^\delta(\ln 2\sin t)^2\,\mathrm{d}t\sim \frac{2}{\pi}\int_0^\delta(\ln 2t)^2\,\mathrm{d}t=\frac{1}{\pi}\int_0^{2\delta}(\ln s)^2\,\mathrm{d}s.$$
而后者收敛，所以 $0$ 不是积分的奇点，从而
$$\E Y_k^2 = \sigma^2<\infty.$$
记 $S_n=\sum_{k=1}^n Y_k$，根据中心极限定理，我们有
$$\frac{S_n}{\sigma\sqrt{n}} \xrightarrow{\ d\ } Z\sim N(0,1).$$
于是对任意 $M>0$，
$$\lim_{n\to\infty}\mathbb{P}\left(\frac{S_n}{\sigma\sqrt{n}}<-M\right) =\mathbb{P}(Z<-M) = p_M > 0.$$
定义事件
$$A_n = \left\{\frac{S_n}{\sigma\sqrt n}<-M\right\}.$$
根据 Fatou 引理，
$$\mathbb{P}(A_n​\ {\rm i.o.})\ge \varlimsup_{n\to\infty}\mathbb{​P}(A_n​)=p_M​>0.$$
另一方面 $\{A_n​\ {\rm i.o.}\}$ 属于尾事件，根据 Kolmogorov 0-1 律，$\mathbb{P}(A_n​\ {\rm i.o.})=1$。

对 $M=1, 2,\ldots$ 求交，我们得到
$$\mathbb{P}\left(\varliminf_{n\to\infty}\frac{S_n}{n}=-\infty\right)=1.$$
使用同样的论证我们也可以得到
$$\mathbb{P}\left(\varlimsup_{n\to\infty}\frac{S_n}{n}=+\infty\right)=1.$$
即 $S_n$ 以概率 1 在 $(-\infty, +\infty)$ 之间振荡，从而 $\exp S_n$ 以概率 1 在 $(0, +\infty)$ 之间振荡。

现在回到等式
$$R_n=R_0\exp S_n.$$
对几乎处处的 $\omega$，$\exp S_n$ 可以任意接近 0，而且这种事情会发生无穷多次，所以飞船以概率 1 回到原点。
