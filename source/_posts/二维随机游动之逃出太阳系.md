...
$$
事实上用一个旋转矩阵 $A$ 乘以 $x$ 等同于对 $\theta$ 作变元代换 $\theta'=A^{-1}\theta$，由于 $A$ 是正交矩阵所以 $|\theta'|=|\theta|$ 并且变换的 Jacobian 值是 +1，所以积分不变。于是我们可以取 $x=(R,0)$ 为实轴正向，则 $\theta\cdot x = R\theta_1$。进一步作代换 $u=R\theta$ 可得
$$I_1 = \frac{1}{\pi^2}\int_{|u|\le R\pi}\frac{1-\cos u_1}{|u|^2}\,\mathrm{d}u.$$
其中 $u_1$ 是 $u$ 的 $x$ 分量。

将积分拆成 $|u|\le 1$ 和 $1<|u|\le R\pi$ 两部分：

+ 在 $|u|\le1$ 时，由于在 0 附近，
$$\frac{1-\cos u_1}{|u|^2}\sim\frac{u_1^2/2}{|u|^2}\le\frac{1}{2}.$$
所以被积函数在单位圆内连续有界，这部分积分是个有限的常数，可以不用管。
+ $1<|u|\le R\pi$ 对应的积分等于
$$\frac{1}{\pi^2}\int_{1<|u|\le R\pi}\frac{1}{|u|^2}\,\mathrm{d}u - \int_{1<|u|\le R\pi}\frac{\cos u_1}{|u|^2}\,\mathrm{d}u.$$
极坐标代换以后第一项等于
$$\frac{2}{\pi}\int_1^R\frac{1}{r}\,\mathrm{d}r = \frac{2}{\pi}\ln R + O(1).$$
这一项贡献了 $I_1$ 的主项。\
同样使用极坐标代换，第二项等于
$$\frac{1}{\pi^2}\int_{1}^{\pi R} \frac{1}{r} \left( \int_{0}^{2\pi} \cos(r \cos\alpha)\, \mathrm{d}\alpha \right) \,\mathrm{d}r=\frac{2}{\pi}\int_1^{\pi R}\frac{J_0(r)}{r}\,\mathrm{d}r.$$
其中 $J_0(r)$ 是零阶第一类贝塞尔函数。熟知 $J_0(r)$ 在 $r\to\infty$ 时的渐进阶是 $O(r^{-1/2})$，所以这部分的积分收敛。
...
上面这些讨论基本上是我对 Spitzer 著作中内容的重新表述，利用局部中心极限定理可以证明极限的误差项大约是 $O(|x|^{-2})$ 量级的，即
...
记上面的分母为 $g=\mathbb{E}_x\left[a(S_{\tu})|\tu<\td\right]$，则 $g$ 是势核 $a(\cdot)$ 在圆周 $\partial B(r)$ 上关于一个条件概率测度的积分，而我们已经看到势核 $a(y)$ 是近似球对称的，它只依赖于 $|y|$。当随机游动刚好走到 $\partial B(r)$ 时，其位置 $y$ 未必正好落在圆周上，但是满足 $r\le |y|< r+1$。我们滥用一点记号，对任意的实数 $r>0$，记
$$a(r) = \frac{2}{\pi}\ln r + \frac{2\gamma+\ln8}{\pi},$$
与 @Pre:axorder 同样的道理，当 $r\le |y|< r+1$ 时有
$$|a(y) - a(r)| = O(1/r).$$
