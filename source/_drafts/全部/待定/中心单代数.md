...
$$z=a_1\otimes b_1+a_2\otimes b_2+\cdots+a_m\otimes b_m=x_1\otimes b_1'+\cdots + x_k\otimes b_k'.$$
...
$$z=x_1\otimes b_1'+\cdots + x_kb_k'\in C_A(R)\otimes C_B(S).$$
反向包含得证。$\blacksquare$
...
最后设 $C$ 的中心化子为 $D=C_A(C)$，对单子代数 $C$ 应用结论 2，得到
$$\dim_F A =\frac{\dim_F C}{\dim_F D}.$$
对单子代数 $B$ 应用结论 2，得到
$$\dim_F A =\frac{\dim_F B}{\dim_F C}.$$
比较两式得知 $\dim_F B = \dim_F D$。然而 $B\subseteq D=C_A(C)$，二者维数相同因此必然相等，3 得证。$\blacksquare$
