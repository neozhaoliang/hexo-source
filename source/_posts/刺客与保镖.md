...
由于光路的可逆性，这个话也可以反过来说，即虚像房间中的一点可能来自真实房间中若干个不同的实际位置。我们后面会证明这样的位置最多有 6 种不同的可能。

所以在正六边形房间的情形：虽然虚像仍然由 6 个不同的格点组成；封锁每个格点也还是需要 4 名保镖；但是对一个特定的保镖和他的虚像格点，当将该格点折叠回真实房间时，最终的实际位置有多达 6 种可能。所以保镖数目要乘以 6，即总共需要 4 x 6 x 6 = 144 名保镖。
...
然而 $p$ 并不是真实房间中唯一可以生成 $q+L$ 的点。记 $S=\{y\cdot p\mid y\in D_3\}$，$S$ 都是真实房间中的点，并且最多包含 6 个不同的点。对 $y\cdot p\in S$，用 $xy^{-1}\in D_3$ 作用在 $y\cdot p$ 上，得到 $x\cdot p$，然后按照 $L$ 进行平移，就可以得到格点 $q+L$。这就证明了 $p$ 的一个虚像格点可能来自房间内多达 6 个不同的位置。我们需要在所有 6 个位置布置保镖，所以最终保镖的数目要乘以 6。