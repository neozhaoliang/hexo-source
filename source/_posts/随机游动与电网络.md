...
:::{.definition}
设 $f:V\to\mathbb{R}$。定义**拉普拉斯算子** $\Delta: V\to\mathbb{R}$ 为
$$(\Delta\, f)(x) = f(x) - \sum_{y\sim x}\frac{c(x,y)}{c(x)}f(y).$$
...
$v(x)$ 满足边界条件 $v(a)=1, v|_Z\equiv0$。利用单步转移分析容易看到 $v(x)$ 在任何 $x\notin \{a\}\cup Z$ 处调和，
所以 $v(x)$ 是电势。于是一般边界条件对应的电势为
$$h=(\alpha-\beta)v + \beta.$$
...
注意等效电阻与 @Pre:v-div-i 中的电势 $h$ 无关，是完全由网络自身决定的。
